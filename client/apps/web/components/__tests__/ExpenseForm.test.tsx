...
describe('ExpenseForm', () => {
  ...
  it('submits transaction with correct data', async () => {
    ...
    // Account is already selected (Cash) by default
    ...
    // Enter Note
    await user.type(screen.getByPlaceholderText(/Add a note/i), 'Lunch');

    // Submit
    const submitBtn = screen.getAllByText(/Guardar Gasto/i)[0];
  ...
  it('shows real-time budget impact', async () => {
    const user = userEvent.setup();
    render(<ExpenseForm />);
    
    // Select Category (Food, remaining 100)
    const categoryTrigger = screen.getByText('Select category').closest('button');
    if (!categoryTrigger) throw new Error('Category trigger not found');
    await user.click(categoryTrigger);
    await user.click(await screen.findByText('Food'));
    ...
    // Verify current and projected balance
