...
test('Take New Expense Screenshot', async ({ page }) => {
  ...
  await amountInput.pressSequentially('42.00', { delay: 50 });

  const noteInput = page.getByRole('textbox', { name: 'Note (optional)' });
  await noteInput.click();
  await noteInput.pressSequentially('Weekly groceries', { delay: 50 });
